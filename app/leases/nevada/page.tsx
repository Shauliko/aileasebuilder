...
const CONFIG = STATE_CONFIGS.find(s => s.slug === "nevada")!;
...
export const metadata: Metadata = {
  ...
  description: `Generate a ${stateName}-specific residential lease that follows ${stateName} landlord–tenant law. Lawyer-grade, AI-generated.`,
  alternates: { canonical: pageUrl },
  openGraph: {
    title: `${stateName} Residential Lease Agreement`,
    description: `Create a ${stateName}-compliant lease in minutes with AI Lease Builder.`,
    url: pageUrl,
    type: "article",
  },
  twitter: {
    ...
    title: `${stateName} Residential Lease Agreement`,
    description: `Generate a ${stateName} rental lease using AI.`,
  },
};
...
export default function Page() {
  const jsonLd = {
    ...
    description: `AI-generated residential lease aligned with ${stateName} landlord–tenant rules.`,
  ...
  return (
    <>
      <Script id="nv-jsonld" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />
      <Script id="nv-breadcrumbs" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbs) }} />
      ...
      <section className="mb-10 space-y-3 text-sm">
        ...
        <div className="mt-4 flex flex-wrap gap-3 text-xs text-gray-300">
          ...
          {relatedStates.map(st => (
            <a key={st.slug} href={`/leases/${st.slug}`} className="text-emerald-400 underline-offset-2 hover:underline">
              {st.label}
            </a>
          ))}
        </div>
